...
BASE_URL = "https://www.propertybook.co.zw/residential/to-rent"
OUTPUT_PATH = Path(__file__).resolve().parents[1] / "data" / "rentals.json"
USER_AGENT = (
...
def parse_listing(listing) -> Optional[Dict]:
    ...
    if not listing_url:
        for anchor in listing.select("a[href]"):
            ...
            if "/listings/to-rent/" in href and "/listed-agencies/" not in href:
                listing_url = href
                break
