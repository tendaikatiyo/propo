...
OUTPUT_PATH = Path(__file__).resolve().parents[1] / "data" / "property_co_sales.json"
USER_AGENT = (
...
def is_sale_listing(listing_url: str, offers: Dict) -> bool:
    if "/for-rent/" in listing_url:
        return False
    offer_type = str(offers.get("@type") or "")
    if "Lease" in offer_type:
        return False
    return "/for-sale/" in listing_url or "sale" in offer_type.lower()
...
def jsonld_to_record(listing: Dict) -> Optional[Dict]:
    ...
    if not is_sale_listing(listing_url, offers):
        return None
...
def save_json(listings: List[Dict]) -> None:
    ...
    print(f"Saved {len(listings)} sale listings to {OUTPUT_PATH}")
...
def main() -> None:
    ...
    if collected:
        ...
        print("No sale listings were collected.")
