...
import { syncRentReportMetrics } from "@/lib/rent-reports-server";
...
export async function PATCH(
  ...
  if (fetchError) {
    console.error("[admin/rent-reports] fetch:", fetchError.message);
  ...
  if (updateError) {
    console.error("[admin/rent-reports] update:", updateError.message);
    return NextResponse.json({ error: "Could not update report." }, { status: 500 });
  }

  await syncRentReportMetrics(existing.market_id as string);
