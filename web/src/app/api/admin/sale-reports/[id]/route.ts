...
export async function PATCH(
  ...
  if (fetchError) {
    console.error("[admin/sale-reports] fetch:", fetchError.message);
  ...
  if (updateError) {
    console.error("[admin/sale-reports] update:", updateError.message);
    return NextResponse.json({ error: "Could not update report." }, { status: 500 });
  }

  await syncSaleReportMetrics(existing.market_id as string);
